# Grubhub | Payment Data Extraction Prompt

Description: Here's a comprehensive prompt to extract weekly performance metrics from Grubhub payment details:
Prompt Text: You are analyzing **Grubhub payment data** for a restaurant chain. Extract weekly performance metrics from the CSV file covering Monday through Sunday using CONSERVATIVE ATTRIBUTION methodology.

**IMPORTANT: Only include orders where Channel = "Marketplace" AND Order Status = "Completed" in all calculations**

& All calculations should be only taking into account Capriotti's Sandwich Shop - exclude all results from Wing Zone under store name

--- 

**UPDATED ATTRIBUTION METHODOLOGY (GRUBHUB):**

1. **Order Filtering**  
   - ONLY count orders where Channel = "Marketplace" AND Order Status = "Completed"  
   - Exclude ALL orders from other channels (white-label, corporate, etc.)  
   - Exclude: Cancelled, Unfulfilled, Refund, Disputed

2. **Sales Calculation (Updated)**  
   - **Total Sales (Net):** Use the "Sales (excl. tax)" field to match Grubhub merchant reporting  
   - **Marketing Driven Sales:** Sum "Sales (excl. tax)" for orders with promotional offers (including all types of offers and credits)  
...
4. **Net Payout Calculation (Updated)**  
   - **Net Payout $ (All Statuses):** Sum ALL "Total Payout" values for ALL order statuses (Completed, Refund, Cancelled, Unfulfilled, Disputed)  
   - Includes negative payouts for refunds/adjustments and positive payouts for completed orders
...
5. **Marketing Attribution**  
   ...
   - **Organic Sales:** All other completed orders without promotional discounts, marketing credits, or third-party contributions
...
**NOTE:** Wherever “Offer/Discount Value”, “Discount Value”, “Sales from offers”, or offer-related metrics are calculated, include “Marketing Credits” and “Third-party Contribution” credits alongside ALL other promotional discounts, and update any formulas or aggregation logic accordingly.
...
[doordash_weekly_analysis_2025-09-08_to_2025-09-14.xlsx](Grubhub%20Payment%20Data%20Extraction%20Prompt%20287d3ff018e78068b4e6c2de98118c0e/doordash_weekly_analysis_2025-09-08_to_2025-09-14.xlsx)
