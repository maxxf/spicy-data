...
- **Corporate Locations Report (October 28, 2025)**: Dedicated tracking of 16 Nevada/Arizona test locations with hardcoded store IDs (AZ900482, NV008-NV900478). Calculation logic verified 100% accurate via direct DB queries matching API responses exactly. All discrepancies between report and platform CSV files traced to incorrect original CSV uploads, not calculation errors. Comprehensive validation tools created: `scripts/validate-corp-data.sh` for 8-week summary and `scripts/debug-corp-locations.sh` for detailed platform breakdowns. Full documentation in `CORP_LOCATIONS_DATA_VALIDATION.md`.
